...
import error.InstantiationRuntimeError
  ...
  def getMatrixSize: (Option[Int], Option[Int]) = {
    operation match {
      ...
      case Minimum | Maximum | Norm2 => {
        if (matrix.cols == Some(1)) (Some(1), Some(1))
        else (matrix.rows, Some(1))
      }
    }
  }
...
case class rand(numRows: ScalarRef, numColumns: ScalarRef, mean: ScalarRef = scalar(0), std: ScalarRef = scalar(1)) extends FunctionMatrixTransformation {
  val rows = scalarRef2Int(numRows)
  val cols = scalarRef2Int(numColumns)

  def instantiate(args: Executable*): rand = {
    rand(numRows.instantiate(args: _*), numColumns.instantiate(args: _*), mean.instantiate(args: _*),
...
sealed trait ScalarRef extends ExpressionExecutable {
  ...
  def times(matrix: Matrix) = { ScalarMatrixTransformation(this, matrix, Multiplication) }
  def div(matrix: Matrix) = { ScalarMatrixTransformation(this, matrix, Division) }
  def div(other: ScalarRef) = { ScalarScalarTransformation(this, other, Division) }
  def minus(other: ScalarRef) = { ScalarScalarTransformation(this, other, Subtraction) }
